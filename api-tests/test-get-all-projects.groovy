...
/**
 * Test script for the getAllProjects API endpoint
 * Uses common functionality from ApiTestBase pattern
 */
...
final String fullUrl = API_GATEWAY_URL + projectsEndpoint
...
println "Path handling: StripPrefix=1 filter should convert /api${projectsEndpoint} to ${projectsEndpoint}"
println "\n==============================\n"
...
try {
    ...
    try {
        ...
        def projects = response.getBody()
        def responseBodyJson = JsonOutput.toJson(projects)
        println "DEBUG: Response body length: ${responseBodyJson.length()} characters"
        
        println "\n========== RESULTS ==========\n"
        println "Projects retrieved: ${projects.size()}"
        ...
        switch (statusCode) {
            case 404:
                ...
                println "1. The endpoint path is incorrect (${projectsEndpoint})"
                ...
                println "- Try accessing the endpoint directly on the Spring Cloud Gateway (13.52.157.48:8090/api${projectsEndpoint})"
                break
            case 502:
            case 503:
            case 504:
    ...
    if (e instanceof org.springframework.web.client.ResourceAccessException) {
        ...
        println "Command: curl -v http://13.52.157.48:8090/api${projectsEndpoint}"
    }
